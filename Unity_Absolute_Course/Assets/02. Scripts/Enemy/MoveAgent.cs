...
using UnityEngine;
//네비게이션 기능을 사용하기 위해 주가해야 하는 네임 스페이스
...
public class MoveAgent : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if(group != null)
        {
            //waypointgroup 하위에 있는 모든 트렌스폼 컴퍼넌트를 추출한 후 리스트 타입의 waypoints 배열에 축가
    ...
    {
        //최단거리 경로 계싼이 끝나지 않았으면 다음을 수행하지 않음
    ...
    void Update()
    {
        ...
        {
            //NavMeshAgent가 가야 할 방향 백터를 쿼터니언 타입의 각도로 변환
            Quaternion rot = Quaternion.LookRotation(agent.desiredVelocity);
            //보간 함수를 사용해 점진적으로 회전시킴
            enemyTr.rotation = Quaternion.Slerp(enemyTr.rotation, rot, Time.deltaTime * damping);
        }
        //순찰 모드가 아닐경우 로직을 수행하지 않음 
        if (!_patrolling) return;
        //NavMeshAgent가 이동하고 있고 목적지에 도착했는지 여부를 계싼
